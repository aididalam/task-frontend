...
const TaskColumn = ({ status, tasks, updateTask, columnStyle }) => {
  ...
  return (
    <div
      ref={drop}
      className={`flex-shrink-0 w-full sm:w-[100%] min-h-[400px] p-4 sm:p-5 rounded-xl border ${columnStyle} transition-all duration-300 column`}
    >
      <h3 className="font-bold text-lg sm:text-xl mb-2 sm:mb-4 text-gray-800">{status}</h3>
      <div className="space-y-2 sm:space-y-4 tasklist">
        {tasks.map((task) => (
